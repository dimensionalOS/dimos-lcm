...
export class Grid {
  ...
  constructor(init?: Partial<Grid>) {
    ...
    this.column_count = init?.column_count ?? 0;
    this.cell_size = init?.cell_size ?? new Vector2();
    this.row_stride = init?.row_stride ?? 0;
    this.cell_stride = init?.cell_stride ?? 0;
    this.fields = init?.fields ?? [];
    this.data = init?.data ?? new Uint8Array(0);
  }

  static decode(data: Uint8Array): Grid {
    ...
    ({ offset } = Grid._decodeOne(view, offset, result));
    return result;
  }

  static _decodeOne(view: DataView, offset: number, target: Grid): { offset: number } {
    ...
    target.column_count = view.getInt32(offset, false);
    offset += 4;
    target.cell_size = new Vector2();
    ({ offset } = Vector2._decodeOne(view, offset, target.cell_size));
    target.row_stride = view.getInt32(offset, false);
    offset += 4;
    target.cell_stride = view.getInt32(offset, false);
    offset += 4;
  ...
  _encodeOne(view: DataView, offset: number): number {
    ...
    view.setInt32(offset, this.column_count, false);
    offset += 4;
    offset = this.cell_size._encodeOne(view, offset);
    view.setInt32(offset, this.row_stride, false);
    offset += 4;
    view.setInt32(offset, this.cell_stride, false);
    offset += 4;
  ...
  _encodedSize(): number {
    ...
    size += 4;
    size += this.cell_size._encodedSize();
    size += 4;
    size += 4;
    for (let i0 = 0; i0 < this.fields_length; i0++) {

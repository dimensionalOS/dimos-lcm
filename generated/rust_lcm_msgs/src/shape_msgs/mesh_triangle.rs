...
impl MeshTriangle {
    pub const HASH: i64 = 0x2FF7303A4EEA918Du64 as i64;
    pub const NAME: &str = "shape_msgs.MeshTriangle";
    ...
    pub fn encode_one<W: Write>(&self, buf: &mut W) -> io::Result<()> {
        for v0 in self.vertex_indices.iter() {
    ...
    pub fn decode_one<R: Read>(buf: &mut R) -> io::Result<Self> {
        let vertex_indices = {
            let mut _arr_0 = [0i32; 3];
            for elem in _arr_0.iter_mut() {
                *elem = buf.read_i32::<BigEndian>()?;
            }
            _arr_0
        };
        Ok(Self {
    ...
    pub fn encoded_size(&self) -> usize {
        let mut size = 0usize;
        size += self.vertex_indices.len() * 4;
        size
    }

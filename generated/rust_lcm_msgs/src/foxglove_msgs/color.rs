...
impl Color {
    pub const HASH: i64 = 0x998136169753C441u64 as i64;
    pub const NAME: &str = "foxglove_msgs.Color";
    ...
    pub fn encode_one<W: Write>(&self, buf: &mut W) {
        buf.write_f64::<BigEndian>(self.r).unwrap();
        buf.write_f64::<BigEndian>(self.g).unwrap();
        buf.write_f64::<BigEndian>(self.b).unwrap();
        buf.write_f64::<BigEndian>(self.a).unwrap();
    }

    pub fn decode_one<R: Read>(buf: &mut R) -> io::Result<Self> {
        let r = buf.read_f64::<BigEndian>()?;
        let g = buf.read_f64::<BigEndian>()?;

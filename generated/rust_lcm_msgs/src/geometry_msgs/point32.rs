...
impl Point32 {
    pub const HASH: i64 = 0x2A14F112C253AC0Cu64 as i64;
    pub const NAME: &str = "geometry_msgs.Point32";
    ...
    pub fn encode_one<W: Write>(&self, buf: &mut W) {
        buf.write_f32::<BigEndian>(self.x).unwrap();
        buf.write_f32::<BigEndian>(self.y).unwrap();
        buf.write_f32::<BigEndian>(self.z).unwrap();
    }

    pub fn decode_one<R: Read>(buf: &mut R) -> io::Result<Self> {
        let x = buf.read_f32::<BigEndian>()?;
        let y = buf.read_f32::<BigEndian>()?;
        let z = buf.read_f32::<BigEndian>()?;
        Ok(Self {
            x,
            y,
            z,

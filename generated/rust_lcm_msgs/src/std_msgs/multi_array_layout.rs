...
pub struct MultiArrayLayout {
    pub dim: Vec<crate::std_msgs::MultiArrayDimension>,
    pub data_offset: i32,
}

impl MultiArrayLayout {
    pub const HASH: i64 = 0x0BF4B5363481D321u64 as i64;
    pub const NAME: &str = "std_msgs.MultiArrayLayout";
    ...
    pub(crate) fn hash_recursive(parents: &mut Vec<u64>) -> u64 {
        ...
        tmphash = tmphash.wrapping_add(crate::std_msgs::MultiArrayDimension::hash_recursive(parents));
        parents.pop();
        // rotate left by 1
    ...
    pub fn encode_one<W: Write>(&self, buf: &mut W) {
        buf.write_i32::<BigEndian>(self.dim.len() as i32).unwrap();
        for v0 in self.dim.iter() {
            v0.encode_one(buf);
        }
        buf.write_i32::<BigEndian>(self.data_offset).unwrap();
    }

    pub fn decode_one<R: Read>(buf: &mut R) -> io::Result<Self> {
        let dim_length = buf.read_i32::<BigEndian>()? as usize;
        let dim = {
            let mut v = Vec::with_capacity(dim_length);
            for _ in 0..dim_length {
                let _elem_0 = crate::std_msgs::MultiArrayDimension::decode_one(buf)?;
                v.push(_elem_0);
            }
            v
        };
        let data_offset = buf.read_i32::<BigEndian>()?;
        Ok(Self {
            dim,
            data_offset,
        })

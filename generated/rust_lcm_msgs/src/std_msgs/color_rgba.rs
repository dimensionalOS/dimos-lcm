...
impl ColorRGBA {
    pub const HASH: i64 = 0x3719400FC6E3011Au64 as i64;
    pub const NAME: &str = "std_msgs.ColorRGBA";
    ...
    pub fn encode_one<W: Write>(&self, buf: &mut W) {
        buf.write_f32::<BigEndian>(self.r).unwrap();
        buf.write_f32::<BigEndian>(self.g).unwrap();
        buf.write_f32::<BigEndian>(self.b).unwrap();
        buf.write_f32::<BigEndian>(self.a).unwrap();
    }

    pub fn decode_one<R: Read>(buf: &mut R) -> io::Result<Self> {
        let r = buf.read_f32::<BigEndian>()?;
        let g = buf.read_f32::<BigEndian>()?;
        let b = buf.read_f32::<BigEndian>()?;
        let a = buf.read_f32::<BigEndian>()?;
        Ok(Self {
            r,
            g,
            b,

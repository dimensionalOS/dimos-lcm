...
impl Duration {
    pub const HASH: i64 = 0x263F39EFA2A9AF63u64 as i64;
    pub const NAME: &str = "builtin_interfaces.Duration";
    ...
    pub fn encode_one<W: Write>(&self, buf: &mut W) {
        buf.write_i32::<BigEndian>(self.sec).unwrap();
        buf.write_i32::<BigEndian>(self.nanosec).unwrap();
    }

    pub fn decode_one<R: Read>(buf: &mut R) -> io::Result<Self> {
        let sec = buf.read_i32::<BigEndian>()?;
        let nanosec = buf.read_i32::<BigEndian>()?;
        Ok(Self {
            sec,
            nanosec,
        })

...
pub struct DiagnosticArray {
    ...
    pub status: Vec<crate::diagnostic_msgs::DiagnosticStatus>,
}

impl DiagnosticArray {
    pub const HASH: i64 = 0x0163C308C500B94Du64 as i64;
    pub const NAME: &str = "diagnostic_msgs.DiagnosticArray";
    ...
    pub(crate) fn hash_recursive(parents: &mut Vec<u64>) -> u64 {
        ...
        tmphash = tmphash.wrapping_add(crate::diagnostic_msgs::DiagnosticStatus::hash_recursive(parents));
        parents.pop();
        // rotate left by 1
    ...
    pub fn encode_one<W: Write>(&self, buf: &mut W) {
        buf.write_i32::<BigEndian>(self.status.len() as i32).unwrap();
        self.header.encode_one(buf);
        for v0 in self.status.iter() {
            v0.encode_one(buf);
        }
    }

    pub fn decode_one<R: Read>(buf: &mut R) -> io::Result<Self> {
        let status_length = buf.read_i32::<BigEndian>()? as usize;
        let header = crate::std_msgs::Header::decode_one(buf)?;
        let status = {
            let mut v = Vec::with_capacity(status_length);
            for _ in 0..status_length {
                let _elem_0 = crate::diagnostic_msgs::DiagnosticStatus::decode_one(buf)?;
                v.push(_elem_0);
            }
            v
        };

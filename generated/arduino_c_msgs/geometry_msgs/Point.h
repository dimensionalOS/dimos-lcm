/*
 * geometry_msgs/Point — Arduino-compatible LCM C encode/decode.
 * Wire format: double + double + double = 24 bytes.
 */
#ifndef DIMOS_ARDUINO_MSG_POINT_H
#define DIMOS_ARDUINO_MSG_POINT_H
...
static inline int dimos_msg__Point__encoded_size(void) { return 24; }

static inline int dimos_msg__Point__encode(void *buf, int offset, int maxlen,
    const dimos_msg__Point *p)
{
    int pos = 0, thislen;
...
static inline int dimos_msg__Point__decode(const void *buf, int offset,
    int maxlen, dimos_msg__Point *p)
{
    int pos = 0, thislen;

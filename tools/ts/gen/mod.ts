...
if (args.stdout) {
  ...
  // Generate root mod.ts with decode() function and pre-populated registry
  const pkgList = [...packages.keys()].sort();
  const imports = pkgList.map(pkg => {
    const pkgPath = pkg.replace(/\./g, "/");
    const alias = pkg.replace(/\./g, "_");
    return `import * as ${alias} from "./${pkgPath}/mod.ts";`;
  }).join("\n");

  const registerCalls = pkgList.map(pkg => {
    const alias = pkg.replace(/\./g, "_");
    return `  _registry.registerPackage(${alias});`;
  }).join("\n");

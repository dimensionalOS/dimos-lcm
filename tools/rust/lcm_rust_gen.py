...
def gen_struct(struct: LcmStruct, all_structs: dict[str, LcmStruct]) -> str:
    ...
    lines.append(f"        *CACHE.get_or_init(|| Self::hash_recursive(&mut Vec::new()))")
    lines.append("    }")
    lines.append("")
    ...
    if not member.dimensions:
        if is_primitive(member.type):
            if member.type == "string":
                ...
                return f"0{rust_type(member.type).replace('u8', '')}" if member.type == "byte" else "0"
        else:
...
def gen_decode(lines: list[str], struct: LcmStruct, length_fields: set[str],
    ...
    lines.append('                format!("Hash mismatch: expected {:016x}, got {:016x}", expected, hash)));')
    lines.append("        }")
...
def gen_encode_one(lines: list[str], struct: LcmStruct, length_fields: set[str],
                   all_structs: dict[str, LcmStruct]):
...
def gen_encode_array(lines: list[str], member: LcmMember, pkg: str,
                     source: str, indent: int, dim_idx: int):
    ind = "    " * indent
    dim = member.dimensions[dim_idx]
...
def gen_decode_one(lines: list[str], struct: LcmStruct, length_fields: set[str],
                   all_structs: dict[str, LcmStruct]):
    has_members = bool(struct.members)
    buf_name = "buf" if has_members else "_buf"
...
def gen_decode_member(lines: list[str], member: LcmMember, pkg: str, var_name: str,
                      indent: int, length_fields: set[str]):
    ind = "    " * indent
...
def gen_decode_array(lines: list[str], member: LcmMember, pkg: str, var_name: str,
                     indent: int, dim_idx: int, length_fields: set[str]):
    ind = "    " * indent
    dim = member.dimensions[dim_idx]
    is_last = dim_idx == len(member.dimensions) - 1
    ...
    if dim.is_constant:
        ...
        if is_last and is_primitive(member.type) and member.type != "string":
            ...
            rt = rust_type(member.type)
            lines.append(f"{ind}    let mut {inner_var} = [{gen_primitive_zero(member.type)}; {size}];")
...
def gen_primitive_zero(lcm_type: str) -> str:
    mapping = {
        "int8_t": "0i8", "int16_t": "0i16", "int32_t": "0i32",
        "int64_t": "0i64", "float": "0.0f32", "double": "0.0f64",
        "byte": "0u8", "boolean": "false",
    }
    return mapping.get(lcm_type, "Default::default()")


def gen_encoded_size(lines: list[str], struct: LcmStruct, length_fields: set[str],
                     all_structs: dict[str, LcmStruct]):
...
def gen_encoded_size_member(lines: list[str], member: LcmMember, pkg: str, prefix: str, indent: int):
    ind = "    " * indent
    fname = rust_field_name(member.name)
    accessor = f"{prefix}{fname}"
...
def gen_encoded_size_array(lines: list[str], member: LcmMember, pkg: str,
                           source: str, indent: int, dim_idx: int):
    ind = "    " * indent
    dim = member.dimensions[dim_idx]
    is_last = dim_idx == len(member.dimensions) - 1
...
def generate_crate(lcm_dir: str, out_dir: str):
    ...
    print(f"Generated {sum(len(v) for v in packages.values())} types "

...
use crate::transport::{Lcm, LcmOptions};
...
impl<T: Send + 'static> Route for TypedRoute<T> {
    ...
    fn try_dispatch(&self, data: &[u8]) {
        match (self.decode)(data) {
            ...
            Err(e) => eprintln!("dimos_lcm: decode error on {}: {e}", self.topic),
        }
    }
}
...
/// High-level wrapper around the LCM transport for use in native modules.
///
            ...
            loop {
                tokio::select! {
                    result = lcm.recv() => match result {
                        // route message through the correct mpsc channel
                        Ok(msg) => {
                            for route in &routes {
                        ...
                        Err(e) => eprintln!("dimos_lcm: recv error: {e}"),
                    },
                    Some((topic, data)) = publish_rx.recv() => {
                        // publish through lcm for any outbound messages
                        if let Err(e) = lcm.publish(&topic, &data).await {
                            eprintln!("dimos_lcm: publish error on {topic}: {e}");
                        }
                    }
                }
            }

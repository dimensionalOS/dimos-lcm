...
from dimos_lcm.geometry_msgs import Quaternion, Transform, TransformStamped, Vector3
...
def main():
    ...
    transform2 = create_test_transform("base_link", "sensor")
    broadcaster.send_transform(transform2)
    ...
    try:
        ...
        result = buffer.lookup_transform("world", "sensor", now, lcm_module=lcm_msgs)
        print("Successfully looked up transform:")
        print(
            f"Translation: ({result.transform.translation.x}, {result.transform.translation.y}, {result.transform.translation.z})"
        )
        print(
            f"Rotation: ({result.transform.rotation.x}, {result.transform.rotation.y}, {result.transform.rotation.z}, {result.transform.rotation.w})"
        )
